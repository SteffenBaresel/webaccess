    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response, String Uid)
        ...
        if(Executions.UserIsPermitted(Uid,"monitoring")) {
            ...
            out.println(""
                    ...
                    + "                    $('#c' + hstid).append('<span style=\"cursor: pointer;\" class=\"ui-taov-tile ' + csscl + '\" title=\"' + $.base64.decode( this.SERVICE_NAME ) + '\" onclick=\"ServiceMenu(\\\'' + hstna + '\\\',\\\'' + hstid + '\\\',\\\'' + this.SERVICE_NAME + '\\\',\\\'' + this.SERVICE_ID + '\\\',\\\'' + this.INSTID + '\\\',\\\'' + this.ACK + '\\\',\\\'' + this.ACKID + '\\\',\\\'' + this.DTM + '\\\',\\\'' + this.DTMID + '\\\');\">' + dtmcl + '</span>');\n"
                    + "                });\n"
                    + "            });\n"
                    + "        },\n"
                    + "        dataType: 'json',\n"
